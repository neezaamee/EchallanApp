    ...
    <div class="card-header bg-light">
        <div class="row flex-between-center">
            <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Dumping Points</h5>
            </div>
            ...
            <div class="row justify-content-between align-items-center">
                <div class="col-sm-auto">
                    <div class="position-relative">
                        <input class="form-control form-control-sm shadow-none search" type="search" placeholder="Search dumping points..." aria-label="search" wire:model.live.debounce.500ms="search" />
            ...
            <table class="table table-sm table-striped table-hover align-middle mb-0 fs--1">
                ...
                <tbody class="list">
                    @forelse ($dumpingPoints as $dp)
                        <tr>
                            <td class="text-muted ps-3">#{{ $dp->id }}</td>
                            <td class="fw-bold text-dark">{{ $dp->name }}</td>
                            <td>{{ $dp->circle->name ?? '—' }}</td>
                            <td><span class="badge badge-soft-warning text-dark fs--2">{{ $dp->circle->city->name ?? '—' }}</span></td>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-danger text-white">
                        ...
                        <button type="button" class="btn-close btn-close-white" wire:click="$set('confirmingDumpingPointDeletion', null)"></button>
                    </div>
                    <div class="modal-body p-4">
                        <p class="mb-0">Are you sure you want to <strong>permanently delete</strong> this dumping point? This action cannot be undone.</p>
                    </div>
                    <div class="modal-footer bg-light">
                        <button class="btn btn-falcon-default btn-sm" wire:click="$set('confirmingDumpingPointDeletion', null)">Cancel</button>
                        <button class="btn btn-danger btn-sm" wire:click="deleteDumpingPoint">Delete Permanently</button>
                    </div>
                </div>
            </div>
        </div>
