    ...
    <div class="card-header bg-light">
        <div class="row flex-between-center">
            <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
                <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0">Provinces</h5>
            </div>
            <div class="col-8 col-sm-auto ms-auto text-end ps-0">
                <div id="table-simple-pagination-actions">
                    @can('provinces:create')
                    <a href="{{ route('provinces.create') }}" class="btn btn-falcon-default btn-sm" type="button">
                        <span class="fas fa-plus" data-fa-transform="shrink-3 down-2"></span>
                        <span class="d-none d-sm-inline-block ms-1">New Province</span>
                    </a>
                    @endcan
                </div>
            </div>
            ...
            <div class="row justify-content-between align-items-center">
                <div class="col-sm-auto">
                    <div class="position-relative">
                        <input class="form-control form-control-sm shadow-none search" type="search" placeholder="Search provinces..." aria-label="search" wire:model.live.debounce.500ms="search" />
            ...
            <table class="table table-sm table-striped table-hover align-middle mb-0 fs--1">
                ...
                <tbody class="list">
                    @forelse ($provinces as $province)
                        <tr>
                            <td class="text-muted ps-3">#{{ $province->id }}</td>
                            <td class="fw-bold text-dark">{{ $province->name }}</td>
                            <td><span class="badge badge-soft-secondary text-dark fs--2">{{ $province->slug }}</span></td>
                            <td class="text-muted">{{ $province->created_at->format('M d, Y') }}</td>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg">
                    <div class="modal-header bg-danger text-white">
                        ...
                        <button type="button" class="btn-close btn-close-white" wire:click="$set('confirmingProvinceDeletion', null)"></button>
                    </div>
                    <div class="modal-body p-4">
                        <p class="mb-0">Are you sure you want to <strong>permanently delete</strong> this province? This action might affect linked cities and circles.</p>
                    </div>
                    <div class="modal-footer bg-light">
                        <button class="btn btn-falcon-default btn-sm" wire:click="$set('confirmingProvinceDeletion', null)">Cancel</button>
                        <button class="btn btn-danger btn-sm" wire:click="deleteProvince">Delete Permanently</button>
                    </div>
                </div>
            </div>
        </div>
